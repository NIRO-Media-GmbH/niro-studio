"""Kopfposition beider Perspektiven (FX3 B + a7 A) auf eine gemeinsame, personenübergreifend einheitliche Stelle legen.
(User 15.09.2026: „Du kannst auch beide verschieben um sie anzugleichen … so wie es am besten aussieht und relativ
einheitlich über alle Personen hinweg ist".)

Aufruf: tools/autocut/venv/bin/python _intern/begradigen/kopf_beide.py [--ausfuehren]
...
2. Gemeinsames Ziel T = (s·X, Y) für alle Personen (s = Blickrichtung: Ludwig/Flammann rechts im Bild, Hein links),
   gesucht auf einem Raster: je Stück kleinster Zoom beider Kameras mit voller Bildfüllung und Kopfraum (Haaransatz bleibt
   im Bild); Bewertung = 90-%-Wert des jeweils größeren Zooms + Mittelwert, leichte Vorliebe für Augen nahe der oberen Drittellinie.
3. Je Item: Begradigung (Rotation/Pitch/Yaw) bleibt, Zoom + Position neu. CTA #22 (ohne a7) auf dasselbe Ziel,
   drittes Stück behält den Punch-in ×1,12.
Ohne --ausfuehren nur Rechnung + Prüfbild kopf_beide_vergleich.jpg. Bericht kopf_beide.json (inkl. Vorher-Werte).
"""
...
W, H = ka.W, ka.H
SEITE = {"FX3_0222": 1, "a7MK4_20260908_0118": 1, "FX3_0223": 1, "a7MK4_20260908_0119": 1, "FX3_0228": -1, "a7MK4_20260908_0752": -1}
KOPFRAUM = 20.0  # px über dem Schädeldach müssen im Bild bleiben
KOPF_OBEN = 0.95  # Schädeldach ≈ 0,95 Gesichtshöhen über der Kopfmitte (Vision-Box: Brauen bis Kinn)
...
def daten():
    ...
    for v in p["V1"]:
        ...
        if not b:
            # CTA #22 ohne a7: eigene Proben holen
            for f in fs:
                jpg = ka.FRAMES / f"b_{f:05d}.jpg"
                ka.frame_holen(v.clip, (v.src_in_f + f - v.rec_in_f) / 25, jpg)
            continue
        stuecke.append({"v1": v, "a7": a7, "b": tuple(np.median(np.array(b), axis=0)), "a": tuple(np.median(np.array(a), axis=0)) if a else None})
    # CTA #22: Proben wurden ggf. gerade geholt → Gesichter nachladen
...
def main() -> None:
    ...
    for s_ in stuecke:
        ...
        punch = abs(wb["ZoomX"] - params[stem_b]["resolve_punch_in"]["ZoomX"]) < 1e-3
    ...
    for stem in ("FX3_0222", "FX3_0223", "FX3_0228"):
        zz = [neu_v1[s_["v1"].rec_in_f]["ZoomX"] for s_ in stuecke if Path(s_["v1"].clip).stem == stem]
        print(f"  {stem}: Zoom {min(zz):.3f}–{max(zz):.3f}")
    for stem in ("a7MK4_20260908_0118", "a7MK4_20260908_0119", "a7MK4_20260908_0752"):
        zz = [neu_v2[a7.rec_in_f]["ZoomX"] for s_ in stuecke for a7 in s_["a7"] if Path(a7.clip).stem == stem]
        print(f"  {stem}: Zoom {min(zz):.3f}–{max(zz):.3f}")
