"""Prüfmischung Sprache + Musik + SFX offline nachbauen und messen (Taxodia, 15.09.2026) — nur zum Messen, nichts in Resolve.

Aufruf: PYTHONDONTWRITEBYTECODE=1 tools/autocut/venv/bin/python _intern/sfx/sfx_mischung_pruefen.py
Stems wie _intern/musik/mischung_pruefen.py (dessen Funktionen werden importiert):
  Sprache A1 mit den in Resolve zurückgelesenen Normalisierungs-Gains (autocut/feinschnitt.json), Musik A2/A3 aus MUSIK_PLAN
  (Pegel + lineare Fades), dazu SFX A4/A5 aus sfx_plan.json (Clip-Gain + lineare Fades, Quelle pfad_nas, nur lesend).
Voice Isolation (nur in Resolve) fehlt in der Simulation.
Messungen (BS.1770, 48 kHz):
  - integrierte Lautheit Mischung mit/ohne SFX, Sprache, Musik; Lautheitsbeitrag der SFX; True Peak (4×), Übersteuerung
  ...
  - Grenze „nie lauter als die Musik-Anhebungen": SFX momentan max < Musik momentan max in den Anhebungen (Titel, Endcard)
Schreibt vorschau_mischung.wav (48 kHz / 24 Bit) und analyse/mischung_sfx.json.
Optional SFX_STEM_CACHE=<Ordner>: Sprach-/Musik-Stems dort zwischenspeichern (Wiederholungsläufe).
"""
...
GRENZE_UNTER_SPRACHE = -25.5   # −26 dBFS + 0,5 dB Toleranz
ANHEBUNGEN = [(345, 482), (6645, 6845)]  # Musik-Anhebungen: Titel (A3 Campagna −7,8 dB), Endcard-Finale (A3 Ikoliks −11,3 dB)
...
SPRACH_SCHWELLE = -40.0  # K-gewichtete Lautheit je Frame (40 ms); Raumton der normalisierten Clips liegt bei ca. −43 … −45
SPRACHE_DEUTLICH = -32.0  # Frames mit deutlich hörbarer Sprache (Median der Wort-Frames −24): dort SFX-Peak < Sprach-Peak
