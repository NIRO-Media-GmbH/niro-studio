...
import proto_messung3 as PM  # noqa: E402

LOOK_C = (1.05, 1.00)
PM.render.__defaults__ = (LOOK_C,)
PM.LOOK = LOOK_C
FACES_DIR = HIER / "proto" / "faces_alle"  # Gesichter wiederverwendet
...
def main():
    ...
    items = [i for i in daten["items"] if i["spur"] == 3]
    ...
    for name, its in je_clip.items():
        ...
        grenze = False
        if grenze:
            hinweise.append("Belichtungsgrenze ±2")
    ...
    json.dump({"look": LOOK_C, "clips": clips, "items": plan}, open(HIER / "proto" / "plan_broll_v3.json", "w"), indent=1, ensure_ascii=False)
    ...
    bogen.save(HIER / "proto" / "uebersicht_broll_v3.jpg", quality=86)
    print("Bogen ok")
