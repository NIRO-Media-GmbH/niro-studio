...
LOOK = (1.00, 0.95)  # Kontrast, Saettigung (Node 03, Startwerte Look-Profil)
PROFIL = dict(haut_L=65.0, neutral_hell_L=88.0, neutral_hell_min=0.05, bild_L=50.0, bild_staerke=0.7, bild_ohne_ab_L=90.0,
              lichter_L=98.0, lichter_zusatz=0.02, grenze_e=2.0, ziel_a=0.0, ziel_b=2.0, neutral_L=(30.0, 95.0),
...
def render(x, v, look=LOOK):
    k, s = look
    y = x + np.array([v["e"] + v["wr"], v["e"], v["e"] + v["wb"]], np.float32) * STOP
    y = k * y + P * (1 - k)
...
def bisekt(fn, ziel, lo=-2.0, hi=2.0, tol=0.01):
    flo, fhi = fn(lo), fn(hi)
    if ziel <= flo:
        return lo
    if ziel >= fhi:
    ...
    for runde in range(3):
        ...
        if hat_gesicht:
            ...
            if nh.mean() >= PROFIL["neutral_hell_min"]:
                ...
                e_anker = "bild"
        clip0 = float((L0[..., 0] >= PROFIL["lichter_L"]).mean())
        geschuetzt = False
        while e > 0:
            clip_e = float((lab(x, {**v, "e": e})[..., 0] >= PROFIL["lichter_L"]).mean())
