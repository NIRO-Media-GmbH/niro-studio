...
def main():
    ...
    for name, its in je_clip.items():
        ...
        grenze = any(abs(b["e"]) >= 1.999 for b in bilder)
        if grenze:
    ...
    json.dump({"look": LOOK_C, "clips": clips, "items": plan}, open(HIER / "proto" / "plan_alle.json", "w"), indent=1, ensure_ascii=False)
    ...
    bogen.save(HIER / "proto" / "uebersicht_alle.jpg", quality=86)
    print("Bogen ok")
