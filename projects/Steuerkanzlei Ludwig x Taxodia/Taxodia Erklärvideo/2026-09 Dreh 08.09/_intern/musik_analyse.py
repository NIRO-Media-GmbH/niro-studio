"""Musik-Analyse für Schnitt-Übergänge (Taxodia, 15.09.2026).
...
Helligkeit (Spektral-Schwerpunkt), Abschnittsgrenzen (Energie-/Klangwechsel auf 8-Takt-Raster gerundet).
Hinweis: gemessen, nicht gehört — Übergänge vor der Abnahme gegenhören.
"""
...
def analyse(pfad: Path) -> dict:
    ...
    for i in range(0, n, step):
        ...
        verlauf.append({"t": round(i / fps_env, 1), "db": round(float(20 * np.log10(r.mean() + 1e-9)), 1),
                        "hell_hz": int(c.mean())})
    # Abschnittswechsel: Novelty aus geglätteter Energie + Schwerpunkt, auf 4-Takt-Raster
    k = int(4 * fps_env)
