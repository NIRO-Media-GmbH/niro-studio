"""Alle Zitate + Timecodes der Rappold-Pläne gegen die Scribe-Wortdaten prüfen.
...
Prüft je Tabellenzeile mit Quelle `… · <Ordner>/<Datei> · mm:ss,d–mm:ss,d`:
  a) Clip existiert im Index, ist FX3 (= Ton-Kamera)
...
ALIAS = [
    (r"\brappoldt\b", "rappold"), (r"\braffholdt\b", "rappold"), (r"\brapolt\b", "rappold"),
    (r"\bcrester\b", "crafter"), (r"\bausgelehnt\b", "ausgeliehen"),
    (r"\bvierundzwanzig sieben\b", "24 7"), (r"\bvierundzwanzig\b", "24"),
]
...
for md in sorted([*PLANS.glob("video-*.md"), *PLANS.glob("99-anhang-*.md")]):
    for line in md.read_text(encoding="utf-8").splitlines():
        ...
        zitate = re.findall(r"„([^“”]+)[“”]", quote_cell)
        if not m:
            if zitate:
            ...
            for part in re.split(r"\s*\[…\]\s*|\s*\[\.\.\.\]\s*", frag):
                ...
                if p not in span:
                    problems.append(
                        ...
                        f"     erwartet: {p[:100]}\n"
                        f"     gefunden: {span[:160]}")
        # Sprecher nur im engen Fenster (ohne Toleranz) werten
...
print(f"{checked} Quellenangaben geprüft in {PLANS}.")
if warnings:
