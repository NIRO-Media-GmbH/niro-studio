...
def main() -> None:
    ...
    for r in probe:
        ordner = unicodedata.normalize("NFC", r["ordner"])   # NAS liefert NFD-Umlaute
        if filt and filt not in ordner:
            continue
    ...
    for ordner, clips in sorted(folders.items()):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for ci, c in enumerate(clips):
                ...
                draw.text((6, y + 40), f"Uhr ~{c['uhrzeit_lokal_ca']}", fill=(90, 90, 90), font=font)
