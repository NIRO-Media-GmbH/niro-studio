"""Abschlussprüfung der Library (nur lesend).
...
from config import DATA, LIBRARY, NAS_ROOT
...
def main():
    ...
    ap.add_argument("--scan-tsv", required=True)
    args = ap.parse_args()
    ...
    for p, (dauer, titel) in ergebnisse.items():
        ...
        if dauer < 25:
            fehler.append(f"kürzer als 25 s: {p}")
        if not titel and not p.endswith(".aac"):
            fehler.append(f"kein Titel-Tag: {p}")

    scan = {}
    for line in open(args.scan_tsv, encoding="utf-8"):
        size, mtime, rel = line.rstrip("\n").split("\t", 2)
        scan[str(NAS_ROOT) + rel[1:]] = (int(size), int(mtime))
