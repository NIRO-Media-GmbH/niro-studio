...
def test_place_v2_compact_and_v1_plan_rejected(charge_dir, capsys):
    ...
    # Schluss-Review M4 (Task 8): ein Clip mit ZWEI geretteten Abschnitten — die Ausgabe zählt Abschnitte, nicht Clips
    gerettet = _broll_clip(charge_dir.parent, "FX3_3.MP4", "Detail", "ohne Person", "tele")
    gerettet["stabil_quelle"] = {"ruhig_max_px": 0.15, "bewegung_max": 2.0, "stabil_min_s": 2.0,
                                 "config_hash": TM.config_hash(ch.config["telemetrie"]), "laeufe": [[0.0, 12.0, 0.05, 0.3]]}
    gerettet["abschnitte"] = [{**gerettet["abschnitte"][0], "von_s": von, "bis_s": bis, "verwendbar": False, "maengel": [],
                               "stabil": [[float(von), float(bis), 0.05, 0.3]]} for von, bis in ((0, 6), (6, 12))]
    ...
    assert "2 gerettete Abschnitte" in out and "davon" not in out, out
    # Ledger B2: ein lesbarer Satz mit dem ganzen Weg — erst neu messen, dann den Nachlauf
    assert "mit stabil mit" not in out and out.index("autocut_telemetrie.py") < out.index("autocut_index_sections.py"), out
