"""Vorlage (Stand 15.09.2026): Grading-Vorschlag (grading_vorschlag.json) auf die Feinschnitt-Timeline anwenden und gegenprüfen.

Aufruf (tools/autocut/venv/bin/python):
  _intern/color/grading_anwenden.py --test     → nur das erste Item (V1, frühestes) graden, LUT exportieren, mit Modell vergleichen
  ...
  zusätzlich --spuren=V1,V2 oder --spuren=V3   → nur diese Spuren (Standard V1,V2,V3)
  ohne --test/--alle bricht das Skript ab, bevor es sich verbindet (Abweichung vom Original: dort galt ohne Flag --test)

Eingaben: _intern/color/grading_vorschlag.json (skripte/export_vorschlag.py, Format: grading_vorschlag.beispiel.json),
optional _intern/color/broll_trims.json (broll_trims.py; Schlüssel = Start des V3-Items relativ zum Timeline-Start),
geöffnetes, in dieser Session freigegebenes Resolve-Projekt PROJEKT mit der Timeline TIMELINE.
Ausgaben: _intern/color/grading_einsatz.json (je Lauf überschrieben — bei getrennten --spuren-Läufen vorher umbenennen;
Felder projekt, timeline, modus, aktiv, items[spur, start, clip, gruppe, vorher, cdl_gesetzt, lut_gesetzt, lut_readback,
vergleich_modell], gespeichert, fehlgeschlagen), _intern/color/resolve_export/<Gruppe>.cube (ExportLUT, einmal je Gruppe).

Umsetzung (Variante A des Vorschlags): Node 1 = SetCDL (Gesamt-CDL je Kamera/Set, Log vor LUT) + SetLUT Sony LC-709.
Regeln:
- Nur Items der eigenen Feinschnitt-Timeline im freigegebenen Projekt (Name wird vor dem Schreiben gelesen und abgeglichen),
  aktive Farbversion (lokal) des Items; Media-Pool-Clips bleiben unverändert. V1–V3 inkl. deaktivierter Stücke, V4 nie.
- Keine neue Farbversion anlegen: die Stabilisierung könnte an der Version hängen.
- Zuordnung Item → Gruppe über den Clipnamen (GRUPPEN, B-Roll über BROLL_PRAEFIX). Unbekannter Clip → Abbruch; bis dahin
  gesetzte Items bleiben gegradet, grading_einsatz.json wird dann nicht geschrieben.
- Grading erst nach dem letzten Umbau der Timeline anwenden: ersetzte Items verlieren ihren Grade.
- Projekt-Farbmanagement nur lesen (skripte/grade_check.py), nie ändern. Gemessen 15.09.: RCM v2 mit Input = Timeline =
  Rec.709 (Scene), Output Rec.709-A, Clip-Eigenschaften Data Level „Auto", Input Color Space „Project" → die Clips gehen
  unkonvertiert als S-Log3 in den Node; frische Items haben 1 Node ohne LUT und „Version 1".
- Prüfung: Resolve exportiert den Clip-Grade als 33er-LUT; der wird mit CDL → Sony-LUT (colorlib, trilinear) im Bereich echter
  S-Log3-Werte (0,05–0,80) verglichen. ExportLUT klappt nur, wenn Resolve auf der Color-Seite steht (auf der Edit-Seite lieferte
  es 15.09. bei allen 101 Items „ExportLUT fehlgeschlagen", SetCDL/SetLUT und der Readback GetLUT(1) liefen trotzdem). Die Seite
  nie per Skript wechseln, solange der User auf der Edit-Seite arbeitet — den Vergleich (--test) laufen lassen, wenn er selbst
  auf Color steht. Solange ExportLUT scheitert, versucht das Skript es bei jedem Item der Gruppe erneut.
- Wird von begradigen/pruefung_resolve.py als Modul importiert (cdl_fuer, resolve_cdl, LUT_REL): Namen nicht ändern,
  keine Seiteneffekte beim Import.
Herkunft: Taxodia-Charge, _intern/color/grading_anwenden.py
"""
...
# ── ANPASSEN je Charge ─────────────────────────────
PROJEKT = "<Resolve-Projekt>"  # Name des in dieser Session freigegebenen Resolve-Projekts (exakt; Abgleich vor dem Schreiben)
TIMELINE = "<Timeline-Name>"  # Name der Feinschnitt-Timeline, die gegradet wird (exakt, wie fb sie gebaut hat)
GRUPPEN = {  # Clipname ohne Endung → (Person, Kamera) = Schlüssel in grading_vorschlag.json → cdl_je_kamera_und_set; Kamera "FX3"/"a7_IV"
    # "FX3_0001": ("Person A", "FX3"),
}
BROLL_PRAEFIX = "C0"  # Anfang der B-Roll-Clipnamen → Block b_roll_FX3A — Standard (15.09.): Sony-Dateinamen C0001.MP4 …
LUT_REL = "Sony/SLog3SGamut3.CineToLC-709.cube"  # Node-LUT relativ zum Resolve-LUT-Ordner — Standard (15.09.)
# ── Ende ANPASSEN ──────────────────────────────────
...
def cdl_fuer(vorschlag: dict, clipname: str) -> tuple[str, dict]:
    ...
    if stem.startswith(BROLL_PRAEFIX):
...
def main() -> None:
    if "--alle" in sys.argv:
        ...
        raise SystemExit("Modus fehlt: --test (nur das erste Item) oder --alle — ohne Flag wird nichts gegradet.")
