"""Vorlage (Stand 15.09.2026): Sony-LUTs im Resolve-LUT-Ordner vergleichen — Größe, Ecken, Neutralachse bei full-/legal-Eingang.

Aufruf: tools/autocut/venv/bin/python _intern/color/skripte/lut_check.py   (nur lesen, Ausgabe auf stdout)
Keine chargen-spezifischen Werte. Je LUT: Würfelgröße und Domain, Ecken [0,0,0]/[1,1,1], reines Rot, Ausgang für 0 %–500 %
Reflexion als S-Log3-Codewert (full-in CV/1023 und legal-in) und ab welchem Eingang Neutral-Grün > 0,002 wird.
Befund 15.09. (Referenz): SLog3SGamut3.CineToLC-709.cube (v1.08.04, 33er, „full in full out") ist auf der Neutralachse der
sauberste (kein Farbdrift bis 500 %) → Standard-LUT; TypeA (33er, v1.04.04) weicher, Schwarz angehoben, Lichter leicht magenta;
Cine+709 (65er) kontrastreicher, Lichter warm; SLog2-709 Legacy-Kurve; „Sony SLog3 to Rec709.ilut" ist nur eine 1D-Gamma-LUT
ohne Gamut-Wandlung (nicht verwenden).
Herkunft: Taxodia-Session, Scratchpad color/lut_check.py
"""
import numpy as np
