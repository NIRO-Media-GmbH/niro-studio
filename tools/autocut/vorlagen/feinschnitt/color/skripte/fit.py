"""Vorlage (Stand 15.09.2026): CDL-Fit je Set im S-Log3-Raum (vor dem LUT).
Kette je Pixel: Log -> CDL_Kamera/Set -> Look-CDL (Log, Pivot 18 % Grau) -> Sony LC-709 -> Rec.709 -> Lab.
FX3: Neutralisierung (Wand hell + dunkle Kleidung) + Belichtung (Kompromiss Haut-/Wand-Ziel).
...
Aufruf (im Ordner _intern/color/skripte): tools/autocut/venv/bin/python fit.py cfg3.json   (→ fit3.json)
Look-Varianten zum Vergleich (compare.py): fit.py cfg2.json / cfg_b.json / cfg_c.json → fit2.json / fit_b.json / fit_c.json.
Eingaben: cfg*.json (Look k/sat/exp, Ziele t_skin/t_wall/t_skin_broll, w_wall, lam_*, lut, out), samples.npz + ids.json
(measure.py, lutjpg.py). Ausgabe: <cfg.out> mit cfg, look, look_cdl_log, pivot_log, w_pivot_log, sets[Person] (FX3/A7_cdl_angleich,
FX3/A7_cdl_gesamt als [slope r g b, offset r g b, sat], FX3/A7_phys, vorher_nur_LUT, nachher, frames_nachher, cost, nfev) und
broll (FX3A_cdl_angleich, FX3A_cdl_gesamt, FX3A_phys, shots) → gelesen von sheets.py, compare.py, holdout_eval.py, orig_check.py,
export_vorschlag.py.
Standard-Look (15.09.) = cfg3.json: Log-Kontrast 1,20 um 18 % Grau, Saettigung 1,03, Haut L* 70, helle Wand L* 88 (gleich
gewichtet), B-Roll-Haut L* 36; die Voreinstellung ohne cfg (k 1,15 / sat 1,08 / Haut 69) ist nur der erste Stand.
Voraussetzung: je Person genau 6 Paare „<Person>_1…6" mit Haut-Stichproben auf FX3 und a7 (sonst Abbruch).
Robuste Least-Squares (soft-L1) je Set, regularisiert; FX3 bekommt keinen eigenen Kontrast (den setzt nur der Look).
Wird von holdout_eval.py als Modul importiert (get, lab_mean, IDENT, S, _CACHE, de2000, HELLES_HEMD): Namen nicht ändern.
Herkunft: Taxodia-Session, Scratchpad color/fit.py
"""
import json, os, sys
import numpy as np
from scipy.optimize import least_squares
from colorlib import LUMA, LUTDIR, STOP, apply_lut, de2000, disp_to_lab, lin_to_slog3, load_cube

# ── ANPASSEN je Charge ─────────────────────────────
PERSONEN = [  # Personen-Kürzel wie extract.PAIRS; Reihenfolge = Reihenfolge der Sets im Vorschlag
    # "Person A",
]
HELLES_HEMD = [  # Personen mit hellem Hemd: zusätzlicher a7↔FX3-Match der Region „bright" (in measure.py genauso setzen)
    # "Person B",
]
DUNKEL_NICHT_NEUTRAL = [  # Personen, deren dunkle Kleidung farbig ist (z. B. dunkelblau): kein Neutral-Zwang a*/b* = 0
    # "Person B",
]
BR_WALL_OK = set([  # B-Roll-Frame-IDs (ids.json) mit wirklich neutralen, hellen Wandflächen → Weißabgleich-Anker der B-Roll
    # "BR14_C0001",
])
# ── Ende ANPASSEN ──────────────────────────────────
...
def set_residuals(v, person):
    ...
    for i in range(1, 7):
        ...
        if db is not None and da is not None:
            ...
            if person not in DUNKEL_NICHT_NEUTRAL:        # dunkle Kleidung neutral (farbige: DUNKEL_NICHT_NEUTRAL)
                r += list(ldb[1:] * 0.8) + list(lda[1:] * 0.4)
        bb, ba = get(fb, "bright"), get(fa, "bright")
        if person in HELLES_HEMD and bb is not None and ba is not None:
...
def report_set(person, pB, pA, use_look):
    ...
    for i in range(1, 7):
        ...
        for k in ["skin", "dark", "bright"]:
            ...
            if xb is None or xa is None or (k == "bright" and person not in HELLES_HEMD):
                continue
