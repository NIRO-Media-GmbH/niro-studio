"""Vorlage (Stand 15.09.2026): Messregionen je Frame: Haut (Gesichtsbox innen), dunkle Kleidung, helles Hemd, neutrale Wand.
Speichert Pixel-Stichproben im Log-Raum (S-Log3 CV/1023) fuer die Fits + Debug-Masken.

Aufruf (im Ordner _intern/color/skripte):
  tools/autocut/venv/bin/python measure.py                                              → ids.json, samples.npz, faces.tsv
  tools/autocut/venv/bin/python measure.py ids_holdout.json samples_holdout.npz faces_h.tsv   → Hold-out (holdout_extract.py)
Eingaben: <ids>.json (lutjpg.py / holdout_extract.py), frames/*.npy (extract.py), <faces>.tsv (faces-Binärdatei auf lutjpg/
bzw. lutjpg_h/). Ausgaben: <npz> (Schlüssel „<id>|skin/dark/bright/wall", je höchstens 4000 Log-Pixel), <npz>.n.json (Pixelzahlen
je Region), masks/<id>.jpg (Haut rot, dunkel blau, hell grün, Wand gelb), preview/masks_<ids>.jpg (Übersicht).
Regionen (Rechnung auf 960×540 nach LC-709): Haut = innere Gesichtsbox + Hautfilter; dunkle Kleidung = dunkelste 30 % des
Oberkörpers unter der Box (ohne Hände/Arme); helles Hemd = hellste 25 % im Hemdbereich (nur Personen in HELLES_HEMD);
Wand = hell, glatt, chromaarm, außerhalb der Person, nicht geclippt (40 % chromaärmste Kandidaten). Frames ohne Gesicht haben
nur Wand-Stichproben; fit.py braucht je Paar Haut auf beiden Kameras.
Herkunft: Taxodia-Session, Scratchpad color/measure.py
"""
import json, os
import numpy as np
from PIL import Image
from scipy.ndimage import uniform_filter
from colorlib import KB, KR, LUMA, LUTDIR, apply_lut, downscale, label, load_cube, to_u8, yuv_to_rgb

# ── ANPASSEN je Charge ─────────────────────────────
HELLES_HEMD = [  # Personen-Kürzel (extract.PAIRS) mit hellem Hemd → zusätzliche Region „bright"; in fit.py genauso setzen
    # "Person B",
]
# ── Ende ANPASSEN ──────────────────────────────────
...
for fid in ids:
    ...
    for (x0, y0, x1, y1, c) in faces.get(fid, []):
        ...
        skinlike = (y > 0.12) & (y < 0.97) & (disp[..., 0] >= disp[..., 1] * 0.97) & (disp[..., 1] >= disp[..., 2] * 0.80) \
                   & (sat > 0.04) & (sat < 0.65)
        skin |= inner & skinlike
        # strenger Haut-Test fuer den Ausschluss im Oberkoerper (Haende/Arme), damit fast neutrale Oberteile nicht rausfallen
        ...
        if any(fid.startswith(f"{p}_") for p in HELLES_HEMD) and shirt.sum() > 200:
