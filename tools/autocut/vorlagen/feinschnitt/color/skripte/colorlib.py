"""Vorlage (Stand 15.09.2026): Farbrechnung Proxy-YUV -> S-Log3-Codewerte (Resolve-intern, CV/1023) -> CDL -> Sony-LUT -> Rec.709 -> CIELAB.

Bibliothek ohne Ausgabe und ohne chargen-spezifische Werte (kein ANPASSEN-Block). Genutzt von color/grading_anwenden.py,
color/broll_trims.py, den Analyse-Skripten in color/skripte/ und begradigen/linien_messen.py — Funktions- und Konstantennamen
deshalb nicht ändern. Import: sys.path.insert(0, str(<_intern>/"color"/"skripte")); import colorlib as CL
Befunde, auf denen die Rechnung beruht (gemessen 15.09.2026, Sony FX3/FX3A/a7 IV, XAVC 10 Bit, S-Log3/S-Gamut3.Cine):
- Blackmagic Proxy Generator liest die full-range-geflaggten Originale als Full und setzt nach Legal um
  (Y8 = 16 + CV10·219/1023, Fit rms 0,08). Ein Proxy, normal als TV-Range dekodiert, liefert also CV/1023 — genau das,
  was die Sony-LUTs („full in full out") erwarten. Chroma im Proxy ca. 2 % flacher als im Original (alle Kameras gleich).
  Bei neuen Kameras/Proxy-Einstellungen vorher mit skripte/levels_check.py nachmessen.
- apply_lut rechnet trilinear wie Resolve (Projekteinstellung „Trilinear"); Gegenprobe ffmpeg lut3d (trilinear) ↔ numpy:
  mittlere Abweichung 0,0037 (< 1 Codewert 8 Bit).
- STOP: eine Blende in S-Log3 = 261,5·log10(2)/1023 = 0,07695 im normierten Codewert (Belichtungs-Trims als Offset).
Herkunft: Taxodia-Charge, _intern/color/skripte/colorlib.py
"""
import numpy as np
