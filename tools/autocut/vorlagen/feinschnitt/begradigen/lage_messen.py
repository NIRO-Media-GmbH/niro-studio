"""Vorlage (Stand 15.09.2026): Kameralage der Interview-Clips aus den Sony-Metadaten (rtmd) messen.
...
Eingaben: Feinschnitt-Plan über ../feinschnitt_bauen.py (fb.lade/fb.plan: Interview-Items aus V1 + V2_voll, fb.FPS);
  Original-Clips (nur lesend, auch über SMB). Kameras: Sony FX3 und a7 IV (MP4 mit rtmd-Datenspur 0:d:0).
Liest je Interview-Clip an höchstens 6 Stellen (Mitte der genutzten Items, über den Clip verteilt) 1 s der Metadatenspur:
  - Brennweite tatsächlich (Tag 0x8005) und KB-äquivalent (0x8004), Fokusdistanz (0x8001) — RDD-18-Distanzformat
  - Beschleunigungssensor (Satz mit 0xe44b: 3 × int16 je Sample, Skala 0xe449 = LSB pro g, gemessen 8192) → Schwerkraftvektor;
    Achsen x links, y oben, z vorwärts (Rollvorzeichen mit linien_messen.py gegen Bildlinien bestätigt)
  - Gyroskop (0xe43b, Skala 0xe439) → steht die Kamera still (gyro_max ≈ 0 = Stativ)?
Auslesen per ffmpeg -ss <s> -i <Clip> -map 0:d:0 -c copy -t 1 -f data - (exiftool -ee ist über SMB zu langsam).
Schreibt _intern/begradigen/lage.json {Clip-Stamm: {datei, items, messungen: [{sekunde, brennweite_mm, kb_mm, fokus_m,
  acc_mittel_g, acc_streuung_g, gyro_max, imu_samples, neigung_xy_grad, neigung_z_grad}]}}. Nichts in Resolve.
Weiter: proben_waehlen.py → linien_messen.py (Gegenprobe, optional) → parameter_berechnen.py.
Herkunft: Taxodia-Charge, _intern/begradigen/lage_messen.py
"""
...
# ── ANPASSEN je Charge ─────────────────────────────
# keine eigenen Werte: Clips, Messstellen und Framerate kommen aus feinschnitt_bauen.py (fb.lade, fb.plan, fb.FPS)
# ── Ende ANPASSEN ──────────────────────────────────
...
def main() -> None:
    ...
    for clip, items in sorted(clips.items()):
        ...
        stellen = sorted({round((i.src_in_f + i.src_out_f) / 2 / fb.FPS, 1) for i in items})
        if len(stellen) > 6:
