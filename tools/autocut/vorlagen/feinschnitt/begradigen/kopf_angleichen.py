"""Vorlage (Stand 15.09.2026): B-Perspektive (FX3, V1) auf die Kopfposition der A-Perspektive (a7, V2) schieben, Zoom füllt das Bild.

User-Wunsch (15.09.2026): „Verschiebe die B Perspektive so dass der Kopf da liegt wo er bei der A Perspektive ist, nutze
dann Zoom um das gesamte Bild zu füllen".
...
- Transformwerte aller V1/V2-Items live aus Resolve (nur lesen); V1-Stück ↔ a7-Stück über den Feinschnitt-Plan (V2_voll).
  Abbruch, wenn V1/V2 der Timeline vom Plan abweichen (Handänderungen des Users) — erst abstimmen.
...
- Modell aus der Kalibrierung (parameter_berechnen.resolve_h). Stücke ohne a7 (z. B. CTA) bleiben unverändert.
...
Hinweis: die Prüfung „Transform inzwischen geändert" vergleicht mit demselben Schnappschuss vom Skriptstart (liest nicht neu).
Eingaben: ../feinschnitt_bauen.py (fb.lade/fb.plan, fb.PROJEKT = Freigabe, fb.FPS), parameter_berechnen.py (pb),
  Proxys <Clip-Ordner>/Proxy/<Clip-Stamm>.mov, ../gesichtscheck/faces (Vision-Gesichtsboxen).
Methode überholt: nur FX3 zu verschieben brauchte bei hoch sitzenden Köpfen in der Totalen 1,84–2,27× Zoom (verworfen)
→ kopf_beide.py, dann kopf_final.py. Trotzdem Pflicht OHNE --ausfuehren: holt die synchronen Proben-Frames
kopf/frames/b_<Frame>.jpg (FX3) und a_<Frame>.jpg (a7) und ist das Modul `ka` für kopf_beide.py/kopf_final.py
(proxy, frame_holen, h_von, abbilden, simulieren, KEYS, W, H, FPS, PW, PH, FRAMES, INTERN, TIMELINE, fb, pb, RA).
Herkunft: Taxodia-Charge, _intern/begradigen/kopf_angleichen.py
"""
...
# ── ANPASSEN je Charge ─────────────────────────────
TIMELINE = "<Timeline-Name>"  # Feinschnitt-Timeline (wie anwenden.TIMELINE); die Projekt-Freigabe steht in feinschnitt_bauen.PROJEKT
# ── Ende ANPASSEN ──────────────────────────────────

KEYS = ("RotationAngle", "Pitch", "Yaw", "ZoomX", "ZoomY", "Pan", "Tilt")
W, H = pb.W, pb.H  # Timeline-Auflösung (ANPASSEN in parameter_berechnen.py)
FPS = fb.FPS  # Timeline-Framerate (feinschnitt_bauen.py): Quellzeit in s = Quell-Frame / FPS
PW, PH = 960, 540  # Prüf-Frames
PROBEN = 5  # synchrone Zeitpunkte je FX3-Stück; kopf_beide.py/kopf_final.py rechnen fest mit 5 (Start + (k+1)·Dauer/6)
...
def main() -> None:
    ...
    for v in p["V1"]:
        ...
        for k in range(PROBEN):
            ...
            frame_holen(v.clip, (v.src_in_f + f - v.rec_in_f) / FPS, fx3_jpg)
            frame_holen(s.clip, (s.src_in_f + f - s.rec_in_f) / FPS, a7_jpg)
    ...
    # Prüfbild: je FX3-Stück das mittlere synchrone Paar
    kacheln = []
    for e in ergebnis:
