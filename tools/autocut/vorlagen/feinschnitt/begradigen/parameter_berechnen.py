"""Vorlage (Stand 15.09.2026): Transform-Werte zum Begradigen je Interview-Clip berechnen.

Aufruf: tools/autocut/venv/bin/python _intern/begradigen/parameter_berechnen.py
Grundlage:
- lage.json (lage_messen.py): Schwerkraftvektor aus dem Sony-Beschleunigungssensor (Achsen x links, y oben, z vorwärts;
  Rollvorzeichen gegen Bildlinien geprüft) und Brennweite (KB-äquivalent) → Kameraneigung (Rollen + Nicken) je Clip
  (Median der Messstellen).
- Resolve-Modell aus der Kalibrierung (kalibrierung/homographien.json, Rest < 0,3 px), Mitte = Ursprung, y nach unten:
  ...
  P = [[1,0,0],[0,1,0],[2·Yaw/W, −2·Pitch/H, 1]] (Keystone um die Bildmitte, wird zuerst angewendet); Pan + rechts, Tilt + oben.
...
Punch-in (PUNCH) = Begradigung · Zoomfaktor mit Pan-Versatz → "resolve_punch_in" (null, wenn PUNCH = None).
Schreibt parameter.json {Clip-Stamm: {acc_median_g, kb_mm, F_px_4k, kamera_roll_grad, kamera_nick_grad, resolve,
  resolve_punch_in, ausschnitt_verlust_prozent, kontrolle_rest_neigung_grad}}. Nichts in Resolve.
Wird als Modul `pb` geladen (kopf_angleichen, kopf_beide, kopf_final, nachbesserung, kopf_oben_messen): resolve_h, deckt,
W, H, PUNCH nicht umbenennen. Richtwerte 15.09. (Neigung bis 2,7°): Zoom a7 1,017–1,038, FX3 1,060–1,083.
Herkunft: Taxodia-Charge, _intern/begradigen/parameter_berechnen.py
"""
...
# ── ANPASSEN je Charge ─────────────────────────────
W, H = 3840.0, 2160.0  # Auflösung der Feinschnitt-Timeline in px (Pan/Tilt und Keystone gelten in Timeline-Pixeln)
PUNCH = None  # Punch-in-Stück des Feinschnitts (Werte wie feinschnitt_bauen.PUNCH_IN["props"]); None = kein Punch-in
# PUNCH = {"zoom": 1.12, "pan": -200.0}   (zoom = Faktor auf die Begradigung, pan = Versatz in Timeline-Pixeln)
# ── Ende ANPASSEN ──────────────────────────────────
...
def main() -> None:
    ...
    for stem, d in lage.items():
        ...
        # Punch-in-Variante (nur wenn der Feinschnitt ein Punch-in-Stück hat, PUNCH im ANPASSEN-Block)
        punch_in = None
        if PUNCH is not None:
            ...
            punch_in = {"RotationAngle": round(theta, 3), "Pitch": round(pitch, 5), "Yaw": round(yaw, 5),
