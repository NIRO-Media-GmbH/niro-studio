"""Vorlage (Stand 15.09.2026): Tonmischung des Feinschnitts offline nachbauen und nach BS.1770 messen (Sprache gegen Musik).
...
Musik A2/A3 mit Pegel + linearen Fades) und misst nach BS.1770 (K-Filter, 48 kHz):
  ...
  - Abstand Sprache − Musik in 3-s-Fenstern, in denen gesprochen wird (Fenster unter 10 LU werden einzeln gelistet)
  - Pegelsprünge an den Musikübergängen (Musik-Stem, 2 s vor Überblendung vs. 2 s danach, Einbruch in der Blende)
Richtwert Standard (15.09.): Musik unter Sprache im Median ca. 14 LU leiser; Abweichungen über die Pegel in MUSIK_PLAN korrigieren.
Voice Isolation (nur in Resolve) fehlt in der Simulation; sie ändert den Sprachpegel kaum, senkt aber Raumhall/Rauschen.
Eingaben: feinschnitt_bauen.py (Plan, ENDE, MUSIK_PLAN) und _intern/autocut/feinschnitt.json („normalisierung_a1_db" aus dem
Readback von feinschnitt_bauen.py --bauen). Setzt 25 fps und 48 kHz voraus. Schreibt _intern/musik/mischung.json.
Wird als Modul `mp` geladen (SFX-Mischprüfung: mp.fb, mp.lade_audio, mp.k_filter, mp.integriert) — Namen nicht ändern.
Herkunft: Taxodia-Charge, _intern/musik/mischung_pruefen.py
"""
...
# ── ANPASSEN je Charge ─────────────────────────────
# nichts anzupassen: Plan, ENDE und MUSIK_PLAN kommen aus feinschnitt_bauen.py (dessen ANPASSEN-Block)
# ── Ende ANPASSEN ──────────────────────────────────
...
def main() -> None:
    ...
    fs_pfad = INTERN / "autocut" / "feinschnitt.json"
    if not fs_pfad.exists():
        raise SystemExit(f"{fs_pfad} fehlt — erst feinschnitt_bauen.py --bauen (liefert die Normalisierungs-Gains aus Resolve).")
    fs = json.loads(fs_pfad.read_text())
